...
- 名片分享：解析 `手机号` 或 `手机号;姓名;姓氏`，发送真实名片到选中的目标，支持安全分批、批次冷却、联系人缓存、FloodWait 自动暂停续传、账号熔断、停止任务和实时日志
...
- 已查询的联系人关联结果只缓存在本机数据库；已注册结果 30 天后刷新，未注册结果 3 天后刷新

## 已实现 / 待扩展

已实现：API 配置、账号登录/导入/验证/移除、会话拉取与筛选、名片批量分享（安全分批、账号单任务、联系人缓存、限流暂停续传、熔断、停止、脱敏日志）。
