...
- 名片分享：解析 `手机号` 或 `手机号;姓名;姓氏`，默认先导入 Telegram 通讯录再发送，使名片尽可能关联“发消息”按钮；也可手动选择极速直发，并支持安全分批、群慢速独立调度、FloodWait 冷却、持久化账号熔断和实时日志
...
默认模式会调用 `contacts.importContacts`，并在本地缓存“已导入”状态，新增、过期或从未导入的号码才需要再次准备。手动勾选“极速直发”后才跳过导入，此时名片可能没有“发消息”按钮。
...
已实现：API 配置、账号登录/导入/验证/移除、会话拉取与筛选、名片批量分享（默认通讯录关联、可选极速直发、安全分批、账号单任务、群慢速独立调度、限流冷却、持久化熔断、停止、脱敏日志）。
